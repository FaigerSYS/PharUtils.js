...
(function() {
	...
	if (typeof Zlib.Zip !== 'undefined' || typeof Zlib.Unzip !== 'undefined') {
		...
		PharUtils.PharZipConverter = { 
			 ...
			 */
			toPhar: function(zip_data, password) {
				try {
					if (zip_data instanceof Zlib.Unzip) {
						var zip = zip_data;
					} else if (zip_data instanceof Uint8Array) {
						var zip = new Zlib.Unzip(zip_data, {password: password});
					} else {
						var zip = new Zlib.Unzip(toUint8Array(zip_data), {password: password});
					}
				} catch (error) {
					throw Error('Zlib.Unzip creation error: ' + error);
				}
