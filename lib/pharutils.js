...
(function() {
	...
	var PharUtils = {
		 ...
		 * @property {string}               options.stub           - phar bootstrap stub
		 * @property {number}               options.signature_type - phar signature type
		 * @property {string}               options.metadata       - phar metadata
		 * @property {PharUtils.PharFile[]} options.files          - phar files
		...
		Phar: function(options) {
			...
			this.loadFromContents = function(buffer) {
				...
				for (var num = 0; num < files_num; num++) {
					...
					this.files.push(new PharUtils.PharFile(f_name, f_content, f_opts));
				}
			...
			this.saveAsContents = function(as_u8a) {
				...
				manifest_buffer.writeNumber(this.files.length, 4);
				...
				for (var i in this.files) {
					...
					manifest_buffer.writeNumber(crc32(file.getCompressedContents()), 4);
